...
impl Renderer {
    pub fn new(font: Font<'static>, scale: Scale, screen: Size, text: Size) -> Self {
        Self {font, scale, screen, text }
    } 
    pub fn render_dialogue(&self, bg: Option<&DynamicImage>, sprites: &[(&DynamicImage, i64, i64, u8)], cname: &str, content: &str, dialogue_color: [u8; 4]) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
        ...
        for (sprite, x, y, _) in sprites {
            ...
            if left > 0 || top > 0 {
                overlay(
                        &mut image,
                        &(*sprite).clone().crop_imm(left as u32, top as u32, width as u32, height as u32),
...
// impl Scene {
//     pub fn render_dialogue(&self, bg: Option<&DynamicImage>, sprites: &[(DynamicImage, u8)], )
// }
